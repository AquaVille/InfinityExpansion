...
public final class GearTransformer extends AbstractEnergyCrafter implements RecipeDisplayItem {
    ...
    protected void setup(@Nonnull BlockMenuPreset blockMenuPreset) {
        ...
        blockMenuPreset.drawBackground(new SlimefunItemStack("ITEM_BACKGROUND",Material.BLUE_STAINED_GLASS_PANE, "&9Tool Input").item(), new int[] {
                0, 1, 2,
                9, 11,
                18, 19, 20
        });
        blockMenuPreset.drawBackground(new SlimefunItemStack("ITEM_BACKGROUND",Material.BLUE_STAINED_GLASS_PANE, "&9Material Input").item(), new int[] {
                6, 7, 8,
                15, 17,
                24, 25, 26
        });
    ...
    public void update(@Nonnull BlockMenu inv) {
        ...
        if (inputItem == null) { //no input
            ...
            inv.replaceExistingItem(STATUS_SLOT, new SlimefunItemStack("STATUS_SLOT",Material.BLUE_STAINED_GLASS_PANE, "&9Input a tool or piece of gear").item());
            return;

        }

        if (!SF && StackUtils.getId(inputItem) != null) {
            inv.replaceExistingItem(STATUS_SLOT, new SlimefunItemStack("STATUS_SLOT",Material.RED_STAINED_GLASS_PANE, "&cSlimefun items may not have their material changed!").item());
            return;
        }
        ...
        if (inputToolType == null) { //invalid input
            ...
            inv.replaceExistingItem(STATUS_SLOT, new SlimefunItemStack("STATUS_SLOT",Material.BARRIER, "&cNot a tool or piece of gear!").item());
            return;
        ...
        if (inputMaterial == null) { //no material
            ...
            inv.replaceExistingItem(STATUS_SLOT, new SlimefunItemStack("STATUS_SLOT",Material.BLUE_STAINED_GLASS_PANE, "&9Input materials").item());
            return;
        ...
        if (pair == null) { //invalid material
            ...
            inv.replaceExistingItem(STATUS_SLOT, new SlimefunItemStack("STATUS_SLOT",Material.BARRIER, "&cInvalid Materials!").item());
            return;
        ...
        inv.replaceExistingItem(STATUS_SLOT, new SlimefunItemStack("STATUS_SLOT",Material.LIME_STAINED_GLASS_PANE, "&aTool Transformed!").item());
    }
