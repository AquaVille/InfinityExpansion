...
public final class GeoQuarry extends AbstractMachineBlock implements RecipeDisplayItem {
    ...
    protected boolean process(Block b, BlockMenu inv) {
        if (InfinityExpansion.slimefunTickCount() % this.ticksPerOutput != 0) {
            if (inv.hasViewer()) {
                inv.replaceExistingItem(STATUS, new SlimefunItemStack("STATUS",Material.LIME_STAINED_GLASS_PANE, "&aDrilling...").item());
            }
            return true;
        }
        ...
        if (inv.hasViewer()) {
            inv.replaceExistingItem(STATUS, new SlimefunItemStack("STATUS",Material.LIME_STAINED_GLASS_PANE, "&aFound!").item());
        }
        return true;
    }
